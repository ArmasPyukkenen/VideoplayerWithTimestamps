...
export default class TimestampManager{
  constructor({onRewind}){
    ...
    this.rewindBtn.addEventListener('click', e => {
      onRewind(this.info.find(el => el.id == this.activeTimestamp.dataset.id).time)
    })
  }
  ...
  openEditor(timestamp){
    ...
    this.editor.style.left = `calc(${window.getComputedStyle(timestamp).getPropertyValue('left')} - 5px)`;
  ...
  closeEditor(){
    ...
    if(this.activeTimestamp){
      ...
      this.info.find(el => el.id == this.activeTimestamp.dataset.id).content = this.textField.value;
    }
  }
